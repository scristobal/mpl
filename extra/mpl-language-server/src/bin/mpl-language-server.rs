...
use mpl_language_server::{
    ...
    compute_completions_with_params, compute_diagnostics_raw, function_info,
};
...
impl Server {
    ...
    fn run(&mut self, connection: &Connection) -> Result<()> {
        for message in &connection.receiver {
            if !self.handle_message(connection, message)? {
                break;
            }
        }
    ...
    fn handle_request(&mut self, connection: &Connection, request: ServerRequest) -> Result<bool> {
        ...
        if request.method == CompletionRequest::METHOD {
            let (id, params): (_, CompletionParams) = request.extract(CompletionRequest::METHOD)?;
            ...
            let item = match completion {
                CompletionResult::Keywords { span, options } => options
                    ...
                    .map(|item| {
                        ...
                        CompletionItem {
                            ...
                            documentation: Some(Documentation::String(item.info.to_string())),
                ...
                | CompletionResult::ComputeFunctions { span, options } => options
                    ...
                    .map(|item| CompletionItem {
                        ...
                        documentation: Some(Documentation::String(item.info)),
                        text_edit: Some(CompletionTextEdit::Edit(TextEdit::new(
                            span_to_range(text, span),
                            item.label,
                        ))),
    ...
    fn code_actions(&self, params: &CodeActionParams) -> Option<CodeActionResponse> {
        ...
        let actions = compute_diagnostics_raw(text, HashMap::new())
            .iter()
            .filter(|diagnostic| {
                let left = span_to_range(text, diagnostic.span);
                let right = params.range;
            ...
            .flat_map(|diagnostic| {
                ...
                diagnostic.actions.iter().map(move |action| {
                    let mut changes = HashMap::new();
                    changes.insert(
                        params.text_document.uri.clone(),
                        vec![TextEdit::new(
                            span_to_range(text, action.span),
                            action.insert.clone(),
                        )],
                    );
    ...
    fn handle_response(&self, _connection: &Connection, _response: Response) -> Result<bool> {
        Ok(true)
    }
}
...
fn diagnostic_to_lsp(text: &str, item: &DiagnosticItem) -> Diagnostic {
    ...
    Diagnostic::new(
        span_to_range(text, item.span),
        Some(match item.severity {
