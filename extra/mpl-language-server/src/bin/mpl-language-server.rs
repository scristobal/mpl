...
use mpl_language_server::{
    CompletionResult, DiagnosticItem, Severity as MplSeverity, Span,
...
impl Server {
    ...
    fn code_actions(&self, params: &CodeActionParams) -> Option<CodeActionResponse> {
        ...
        let diagnostics = self.diagnostics(text, Some(range_span));
    ...
    fn did_open_txt_doc(
        ...
        let diagnostics = self.diagnostics(&text, None);
    ...
    fn diagnostics(&self, text: &str, span: Option<Span>) -> Vec<Diagnostic> {
        compute_diagnostics_raw(&text, HashMap::new())
            .iter()
            .filter_map(|diagnostic| {
                if let Some(span) = span
                    && !diagnostic.span.intersects(span)
                {
                    return None;
                }
                Some(diagnostic_to_lsp(&text, diagnostic))
            })
            .collect()
    }
