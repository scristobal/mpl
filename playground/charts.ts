// Renders charts into a side panel, one per pipeline step.

import uPlot from "uplot";
import "uplot/dist/uPlot.min.css";
import type { Series } from "./datasets";

const COLORS_LIGHT = ["#2563eb", "#dc2626", "#16a34a", "#d97706", "#9333ea", "#0891b2", "#e11d48", "#65a30d"];
const COLORS_DARK = ["#60a5fa", "#f87171", "#4ade80", "#fbbf24", "#c084fc", "#22d3ee", "#fb7185", "#a3e635"];
...
function tooltipPlugin(): uPlot.Plugin {
  ...
  function setCursor(u: uPlot) {
    ...
    for (let i = 1; i < u.series.length; i++) {
      ...
      lines.push(`<span style="color:${color}">${escHtml(String(s.label ?? ""))}</span>: ${formatted}`);
    }
...
export function renderCharts(container: HTMLElement, entries: ChartEntry[]): void {
  ...
  for (const entry of entries) {
    ...
    label.className = entry.header ? "chart-header" : "chart-label";
