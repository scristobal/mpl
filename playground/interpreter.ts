// MPL interpreter — evaluates the AST against mock datasets.
...
export async function interpret(query: Query): Promise<Series[][]> {
  if ("Simple" in query) return interpretSimple(query.Simple);
  if ("Compute" in query) return interpretCompute(query.Compute);
...
function seriesName(tags: Record<string, string>): string {
  ...
  assert(entries.length > 0, "Cannot name series with no tags");
...
function applyCompute(
  ...
  const fn: ComputeType = op.Builtin;
  assert(left.length > 0, "Left side of compute has no series");
  assert(right.length > 0, "Right side of compute has no series");
  const l = left[0];
  const r = right[0];

  const len = Math.min(l.values.length, r.values.length);
  const ts = l.timestamps.slice(0, len);
  const values: number[] = [];
  ...
  for (let i = 0; i < len; i++) {
    ...
    switch (fn) {
      case "Div":
        assert(rv !== 0, `Division by zero at index ${i} in compute`);
        values.push(lv / rv);
        break;
      case "Mul":
        values.push(lv * rv);
      ...
      case "Avg":
        ...
        break;
      case "Min":
        values.push(Math.min(lv, rv));
        break;
      case "Max":
        values.push(Math.max(lv, rv));
        break;
      default:
        unreachable(fn);
    }
  ...
  return [{ tags: {}, name, timestamps: ts, values }];
}
...
function computeBucketSpec(
  ...
  if (typeof spec === "object" && "Percentile" in spec) {
    const bucketVals = byLe.map(({ le, series: s }) => {
      const pts = windowPts(s, t, windowSec);
      assert(pts.length > 0, `No data points in window at t=${t} for le=${le}`);
  ...
  assert(allPts.length > 0, `No data points in bucket window at t=${t}`);
  switch (spec) {
    case "Count":
...
async function interpretSimple(q: {
  source: Source;
  filters: Filter[];
  aggregates: Aggregate[];
}): Promise<Series[][]> {
  const ds = String(getParam(q.source.metric_id.dataset));
  const metric = String(q.source.metric_id.metric);
  const steps: Series[][] = [];

  let series = await loadSeries(ds, metric);
  steps.push(clone(series));

  for (const f of q.filters) {
    series = applyFilter(series, [f]);
    steps.push(clone(series));
  }

  for (const agg of q.aggregates) {
    series = applyAggregate(series, agg);
    steps.push(clone(series));
  }
...
async function interpretCompute(q: {
  left: Query;
  right: Query;
  name: Metric;
  op: ComputeFunction;
  ...
  const leftSteps = await interpret(q.left);
  const rightSteps = await interpret(q.right);
  const steps: Series[][] = [...leftSteps, ...rightSteps];

  const name = String(q.name);
  const leftSeries = leftSteps[leftSteps.length - 1];
  const rightSeries = rightSteps[rightSteps.length - 1];
  defined(leftSeries, "Left branch of compute query produced no steps");
  defined(rightSeries, "Right branch of compute query produced no steps");
  let series = applyCompute(leftSeries, rightSeries, q.op, name);
