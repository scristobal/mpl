...
use crate::{compile, parse, query::Query};
...
pub fn parse_wasm(query: &str) -> Result<Query, String> {
    parse(query).map_err(|e| e.to_string())
}

#[cfg(feature = "playground")]
/// Parses and checks a query string into a Query object.
#[wasm_bindgen]
pub fn compile_wasm(query: &str) -> Result<Query, String> {
    compile(query).map_err(|e| e.to_string())
}
