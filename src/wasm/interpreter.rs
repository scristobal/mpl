...
use crate::{
    ...
    wasm::steps::{PipeStep, StepNode, parse_steps},
};

/// Nested map of dataset name → metric name → series list.
pub type Datasets = HashMap<String, HashMap<String, Vec<Series>>>;
...
pub fn interpret(pipe_steps: &[PipeStep], datasets: &Datasets) -> Vec<eyre::Result<Vec<Series>>> {
...
fn get_param<T>(p: &Parameterized<T>) -> eyre::Result<&T> {
    match p {
...
fn require_tag(tags: &HashMap<String, String>, key: &str) -> eyre::Result<String> {
    tags.get(key)
        .cloned()
        .ok_or_else(|| eyre!("Missing required tag: {key}"))
}

fn eval_source(src: &Source, datasets: &Datasets) -> eyre::Result<Vec<Series>> {
...
fn evaluate_cmp(tag_val: &str, rhs: &Cmp) -> eyre::Result<bool> {
    match rhs {
...
fn evaluate_filter(s: &Series, filter: &Filter) -> eyre::Result<bool> {
    match filter {
...
fn apply_filter(series: &[Series], filter: &Filter) -> eyre::Result<Vec<Series>> {
...
fn apply_sample(series: &[Series], rate: f64) -> eyre::Result<Vec<Series>> {
...
fn aggregate_columns(values: &[&[f64]], func: TagsType) -> eyre::Result<Vec<f64>> {
...
fn apply_aggregate(series: &[Series], agg: &Aggregate) -> eyre::Result<Vec<Series>> {
    match agg {
...
fn require_arg(arg: Option<f64>, name: &str) -> eyre::Result<f64> {
    arg.ok_or_else(|| eyre!("Map function {name} requires an argument"))
}

fn apply_map(series: &[Series], mapping: &Mapping) -> eyre::Result<Vec<Series>> {
...
fn apply_align(series: &[Series], align: &Align) -> eyre::Result<Vec<Series>> {
    ...
    series
        ...
        .map(|s| {
            ...
            let end = *s.timestamps.last().unwrap();
...
fn apply_group(series: &[Series], group_by: &GroupBy) -> eyre::Result<Vec<Series>> {
    if series.is_empty() {
...
fn compute_percentile(bucket_vals: &[(f64, f64)], p: f64) -> eyre::Result<f64> {
    if bucket_vals.is_empty() {
        bail!("Cannot compute percentile with no buckets");
    }
    let total = bucket_vals.last().unwrap().1;
    if total == 0.0 {
...
fn time_to_seconds(rt: &RelativeTime) -> eyre::Result<f64> {
...
fn apply_bucket(series: &[Series], bucket: &BucketBy) -> eyre::Result<Vec<Series>> {
    if series.is_empty() {
    ...
    for group_series in groups.values() {
        ...
        let end = *group_series[0].timestamps.last().unwrap();
        let mut new_ts = Vec::new();
        let mut t = start;
        while t <= end {
            new_ts.push(t);
...
fn apply_as(series: &[Series], as_: &As) -> eyre::Result<Vec<Series>> {
